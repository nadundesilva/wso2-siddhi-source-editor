...
(function () {
    ...
    var messageHandler = (function () {
        ...
        function initializeWorker(data) {
            ...
            messageHandlerMap[constants.worker.EDITOR_CHANGE_EVENT] = onEditorChange;
            messageHandlerMap[constants.worker.GENERATE_TOKEN_TOOLTIP] = generateTokenTooltipData;
        }
    ...
    var renderer = (function () {
        ...
        renderer.notifyParseTreeWalkingCompletion = function (errors) {
            postMessage(JSON.stringify({
                type: constants.worker.PARSE_TREE_WALKING_COMPLETION,
                data: errors
            }));
        };
        ...
        renderer.notifyDataPopulationCompletion = function (completionData, incompleteData, statementsList) {
            postMessage(JSON.stringify({
                type: constants.worker.DATA_POPULATION_COMPLETION,
    ...
    function ANTLRWalker() {
        ...
        walker.tokenToolTips = [];

        /*
         * Loading modules to be used inside the main siddhi editor js file
         */
        var antlr4 = require(constants.antlr.INDEX);                                                                          // ANTLR4 JS runtime
        var SiddhiQLLexer = require(constants.antlr.ROOT + constants.antlr.SIDDHI_LEXER).SiddhiQLLexer;
        var SiddhiQLParser = require(constants.antlr.ROOT + constants.antlr.SIDDHI_PARSER).SiddhiQLParser;
        var DataPopulationListener = require(constants.antlr.ROOT + constants.antlr.SIDDHI_DATA_POPULATION_LISTENER).DataPopulationListener;
        var SyntaxErrorListener = require(constants.antlr.ROOT + constants.antlr.SYNTAX_ERROR_LISTENER).SyntaxErrorListener;
        var TokenToolTipUpdateListener = require(constants.antlr.ROOT + constants.antlr.SIDDHI_TOKEN_TOOL_TIP_UPDATE_LISTENER).TokenToolTipUpdateListener;
